...
class OptimizerPlus():
    ...
    def __init__(self, data: StitchingData):
        ...
        a_params, b_params = self.set_params(self.homographies, self.reper_idx)
        self.a_params = torch.nn.Parameter(torch.from_numpy(a_params), device=self.device)
        self.b_params = torch.nn.Parameter(torch.from_numpy(b_params), device=self.device)

    def set_params(self, homographies, reper_idx):
        assert np.allclose(homographies[reper_idx], np.eye(3))

        all_homographies = np.array(homographies)
        assert all_homographies.shape == (self.n_tiles, 3, 3)
        a_params = all_homographies[:2, :2].reshape(self.n_tiles, 4)
        b_params = all_homographies[:2, 2]
        assert b_params.shape == (self.n_tiles, 2)

        return a_params, b_params
    
    def get_homographies(self):
        a = self.a_params.view(self.n_tiles, 2, 2)
        b = self.b_params.view(self.n_tiles, 2, 1)
        c = torch.ones((self.n_tiles, 1, 2), device=self.device, dtype=torch.float32)
        d = torch.ones((self.n_tiles, 1, 1), device=self.device, dtype=torch.float32)
    ...
    def preproc_inliers(self):
        ...
        for inlier in self.inliers:
            ...
            query_inliers.append(inlier.xy_i)
            target_inliers.append(inlier.xy_j)
        
        query_idx = torch.tensor(query_idx)
        target_idx = torch.tensor(target_idx)
        query_inliers = torch.tensor(query_inliers)
        target_inliers = torch.tensor(target_inliers)

        padded = pad_sequence(arrays, batch_first=True, padding_value=0.0)

        return query_idx, target_idx, query_inliers, target_inliers
        
    def project(self, xy, H):
        pass

    def compute_re(self):
        homographies = get_homographies()
        re = torch.det(homographies)
        return re
